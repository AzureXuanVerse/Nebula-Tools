...
export function CleanPanel(props: CleanPanelProps) {
    ...
    const typeOptions = () => ([
        { value: 'all', label: t(props.language, 'clean.typeOptions.all') },
        { value: 'items', label: t(props.language, 'clean.typeOptions.items') },
        { value: 'resources', label: t(props.language, 'clean.typeOptions.resources') },
    ]);
                ...
                <div style="display: flex; flex-direction: column; gap: var(--spacing-md);">
                    <div style="display: flex; flex-direction: column; gap: var(--spacing-sm);">
                        <label style="font-size: 12px; font-weight: 500; color: var(--text-secondary);">
                            {t(props.language, 'clean.typeLabel')}
                        </label>
                        <Segmented
                            options={typeOptions()}
                            value={type()}
                    ...
                    {type() !== 'all' && (
                        <MultiSelect
                            ...
                            label={t(props.language, 'clean.itemIdLabel')}
                            options={(items()
                            ...
                            placeholder={t(props.language, 'clean.itemPlaceholder')}
                        />
                    )}
                </div>
            </Card>
